...
\section*{Alternative Rational Parametrization}
...
\begin{equation}
U\left(t\right)=\left[\frac{2t}{1+t^{2}},-\frac{1}{2}-\frac{1-t^{2}}{1+t^{2}},0\right]
\end{equation}
...
\begin{centering}
\caption{Alternative Rational Parametrization\label{fig:3}}
...
\begin{equation}
V\left(t\right)=\left[0,\pm\frac{1}{2}\sqrt{\left(3-t^{2}\right)\left(1+t^{2}\right)},\frac{t^{2}}{2}\right]
\end{equation}
...
Figure \ref{fig:4} shows how both parametrizations project the circular
arc onto the plane despite their different trajectories. The vertical
axis contains the parameter $t$. The slopes of the curves are roughly
equivalent over most of their range - particularly at the turning
point. For this reason, digital realisations of the surface using
either parametrisation have much the same subjective quality.

Good luck to anyone proving that $|UV|=\sqrt{3}$ using my equations.
...
\centering{}\caption{Functions over full range of parameter $t$\label{fig:4}}
\end{figure}
